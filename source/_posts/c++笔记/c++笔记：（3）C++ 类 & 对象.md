...
**数据封装**是面向对象编程的一个重要特点，它防止函数直接访问类类型的内部成员。类成员的访问限制是通过在类主体内部对各个区域标记 public、private、protected 来指定的。关键字 public、private、protected 称为**访问修饰符**。
...
class Box
{
   double width;    //默认情况下，类的所有成员都是私有的。
   public:
      double length;
...
Final Stage Count: 2
```
转自：[c++类&对象|菜鸟教程](http://www.runoob.com/cplusplus/cpp-classes-objects.html)
